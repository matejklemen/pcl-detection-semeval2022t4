...
from src.models.utils import SENTENCE_SENTIMENT_TAGS
...
def process_stanza(stanza_tokenizer: stanza.Pipeline, hf_tokenizer, examples: List[str], max_length):
    tokens_or_words = []
    stanza_examples = [stanza.Document([], text=_curr_example) for _curr_example in examples]

    for curr_doc in stanza_tokenizer(stanza_examples):
        doc_words = []
        for curr_sent in curr_doc.sentences:
            doc_words.append(SENTENCE_SENTIMENT_TAGS[curr_sent.sentiment])
            for curr_word in curr_sent.words:
                doc_words.append(curr_word.text)

        tokens_or_words.append(doc_words)
...
if __name__ == "__main__":
    ...
    nlp = stanza.Pipeline("en", processors="tokenize,sentiment", use_gpu=(not args.use_cpu))
    ...
    tokenizer.add_special_tokens({"additional_special_tokens": SENTENCE_SENTIMENT_TAGS})
