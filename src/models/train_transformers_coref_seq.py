...
from transformers import RobertaTokenizerFast, RobertaPreTrainedModel, RobertaModel, AutoModelForSequenceClassification
from transformers.modeling_outputs import SequenceClassifierOutput
from transformers.models.roberta.modeling_roberta import RobertaClassificationHead

from src.data.utils import load_binary_dataset, PCLTransformersDataset
from src.models.utils import COREF_ENTITY_TAGS, COREF_SEQ_ENTITY_TAGS
...
def convert_words_to_subwords(hf_tokenizer, tokens_or_words, max_length):
...
if __name__ == "__main__":
    ...
    if args.test_path is not None:
        ...
        assert "spacy_tokens" in test_df.columns

    logging.info(f"{train_df.shape[0]} train, {dev_df.shape[0]} dev, "
                 f"{0 if test_df is None else test_df.shape[0]} TEST examples")

    assert "spacy_tokens" in train_df.columns
    assert "spacy_tokens" in dev_df.columns
    ...
    tokenizer.add_special_tokens({"additional_special_tokens": COREF_SEQ_ENTITY_TAGS})
