...
from src.models.utils import COREF_ENTITY_TAGS, optimize_threshold
...
parser.add_argument("--train_path", type=str,
                    default="/home/matej/Documents/multiview-pcl-detection/data/processed/80_10_10/binary_pcl_train_augmented.tsv")
parser.add_argument("--dev_path", type=str,
                    default="/home/matej/Documents/multiview-pcl-detection/data/processed/80_10_10/binary_pcl_tune_augmented.tsv")
...
class CorefRobertaForSequenceClassification(RobertaPreTrainedModel):
...
def convert_words_to_subwords(hf_tokenizer, tokens_or_words, tags, max_length):
    # `tokens_or_words` and `tags` are expected to be aligned!
    ...
    encoded["coref_ids"] = encoded["input_ids"].clone()
    for idx_example in range(len(tokens_or_words)):
        for position, (curr_id, curr_word_id) in enumerate(zip(encoded["input_ids"][idx_example],
            ...
            if curr_word_id is not None:
                encoded["coref_ids"][idx_example, position] = \
...
if __name__ == "__main__":
    ...
    train_enc = convert_words_to_subwords(hf_tokenizer=tokenizer,
                                          tokens_or_words=train_df["spacy_tokens"].tolist(),
                                          tags=train_df["spacy_coref"].tolist(),
    ...
    dev_enc = convert_words_to_subwords(hf_tokenizer=tokenizer,
                                        tokens_or_words=dev_df["spacy_tokens"].tolist(),
                                        tags=dev_df["spacy_coref"].tolist(),
                                        max_length=args.max_length)
    ...
    test_dataset = None
    if test_df is not None:
        test_enc = convert_words_to_subwords(hf_tokenizer=tokenizer,
                                             tokens_or_words=test_df["spacy_tokens"].tolist(),
                                             tags=test_df["spacy_coref"].tolist(),
                                             max_length=args.max_length)
    ...
    if args.optimize_decision == "after_training":
        del model
        model = CorefRobertaForSequenceClassification.from_pretrained(args.experiment_dir, return_dict=True).to(DEVICE)
